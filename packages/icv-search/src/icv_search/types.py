...
class SearchResult:
    ...
    def from_engine(cls, data: dict[str, Any]) -> SearchResult:
        ...
        ``attributesToHighlight`` is requested) and stores the results in
        ``formatted_hits``. For other backends, a top-level ``formatted_hits``
        key is accepted directly.
        ...
        # Meilisearch embeds highlighted versions as a ``_formatted`` key on
        # each hit.  Extract those into a parallel list.
        ...
        clean_hits = [{k: v for k, v in hit.items() if k not in ("_formatted", "_rankingScore")} for hit in raw_hits]

        return cls(
            hits=clean_hits,
            query=str(data.get("query", "")),
            processing_time_ms=int(data.get("processingTimeMs", data.get("processing_time_ms", 0))),
            estimated_total_hits=int(data.get("estimatedTotalHits", data.get("estimated_total_hits", 0))),
